...
** Packages
   ...
   (let* ((packages
           '(auto-compile         ; automatically compile Emacs Lisp libraries
             ...
             flymake-go           ; A flymake handler for go-mode files
             ...
             go-dlv
             go-direx
             go-projectile        ; go package integration for projectile
             gotest               ; Run Go tests and programs from Emacs (>= 24.3)
             magit-gerrit
             paradox
             column-marker
...
** Require
   ...
   (dolist (feature
            '(auto-compile             ; auto-compile .el files
              ...
              column-marker            ; (3) column markers
...
** Visual
   ...
   (safe-diminish "GitGutter" 'git-gutter-mode)
   #+END_SRC
...
** Flycheck
   ...
   Flycheck offers on-the-fly source code error checking and validation.
...
** Presentation-mode
   ...
   (lexical-let ((size 13))
     (defun global-scale-up ()
       ...
       (set-face-attribute
        'default nil
        :font (concat "Source Code Pro-" (number-to-string (incf size)))))

     (defun global-scale-down ()
       (interactive)
       (set-face-attribute
        'default nil
        :font (concat "Source Code Pro-" (number-to-string (decf size))))))
   #+END_SRC
...
** Golang
   ...
   Golang programming mode makes for a powerful Go environment, with live
   compilation and error checking, inline code completion with function
   envelope hinting, dynamic code inspection (via the go-oracle) and coverage
   support.
   ...
   (defun user/go-mode-hook ()
     ...
     (setq compile-command "go build && go test -v -cover")
     (setq gofmt-command "goimports")
     (linum-mode 1)
     (define-key go-mode-map (kbd "C-c C-x k") 'go-coverage)
     (define-key go-mode-map (kbd "C-c C-x f") 'go-test-current-file)
     (define-key go-mode-map (kbd "C-c C-x t") 'go-test-current-test)
     (define-key go-mode-map (kbd "C-c C-x p") 'go-test-current-project)
     (define-key go-mode-map (kbd "C-c C-x b") 'go-test-current-benchmark)
     (define-key go-mode-map (kbd "C-c C-x x") 'go-run)
...
** Eldoc
   ...
   Eldoc settings. Eldoc is a simple way of documenting the item under the
   cursor in supported modes. go-eldoc supports golang!
...
* Key bindings
  ...
  (define-key custom-bindings-map (kbd "M-k")     'helm-mini) ;; no killies!
  ...
  These are my own favorite bindings for a variety of areas.

  Window navigation/splitting

  #+BEGIN_SRC emacs-lisp
  (define-key custom-bindings-map (kbd "M-o") 'other-window)
  (define-key custom-bindings-map (kbd "M-O") (kbd "C-u -1 M-o"))
  (define-key custom-bindings-map (kbd "M-2") 'split-window-below)
  (define-key custom-bindings-map (kbd "M-3") 'split-window-right)
  (define-key custom-bindings-map (kbd "M-0") 'delete-window)
  #+END_SRC

  Mac OS X style keyboard shortcuts

  #+BEGIN_SRC emacs-lisp
  (define-key custom-bindings-map (kbd "s-z") 'undo)
  (define-key custom-bindings-map (kbd "s-s") 'save-buffer)
  (define-key custom-bindings-map (kbd "s-w") 'kill-this-buffer-unless-scratch)
  (define-key custom-bindings-map (kbd "s-l") 'goto-line)
  #+END_SRC
